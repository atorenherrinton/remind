...
describe("Navigation Bar", () => {
	...
	it("renders no button if there is no user", () => {
		store.dispatch(setUid(undefined));
		expect(screen.queryByRole("sign-out")).not.toBeInTheDocument();
	});

	it("renders a button if there is a user", () => {
		store.dispatch(setUid("uP0PcYMj5geFKrFWR8QPE8ON80E2"));
		expect(screen.getByRole("sign-out"));
	});

	it("the text in the button displays Sign Out if there is a user", () => {
		store.dispatch(setUid("uP0PcYMj5geFKrFWR8QPE8ON80E2"));
		expect(screen.getByRole("sign-out")).toHaveTextContent("Sign Out");
	});

	it("clicking the sign out button should remove the user from the store", () => {
		store.dispatch(setUid("uP0PcYMj5geFKrFWR8QPE8ON80E2"));
		userEvent.click(screen.getByRole("sign-out"));
		const updatedUid = store.getState().authenticate.uid;
		expect(updatedUid).toBe(undefined);
	});
});
